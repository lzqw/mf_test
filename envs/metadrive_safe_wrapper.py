...
class SafeMetaDriveSampleWrapper(gym.Wrapper):
    ...
    def step(self, raw_action):
        ...
        safety_failure = bool(crash > 0.0 or out_of_road > 0.0 or (self.safety_cost_termination and cost > 0.0))
        if self.terminate_on_safety_violation and safety_failure:
            terminated = True
            info["failure"] = 1.0
            info["is_success"] = 0.0
        else:
        ...
        info["safety_failure"] = float(safety_failure)
        info["terminated_by_safety"] = float(self.terminate_on_safety_violation and safety_failure)
