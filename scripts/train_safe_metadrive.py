...
from relax.safety.metadrive_cbf_builtin_filter import CBFBuiltinFilterConfig
...
def make_algo(args, obs_dim, act_dim):
    ...
    return SafePullbackRF2SACENTMetaDrive(net, params, gamma=args.gamma, gamma_p=args.gamma_p, lr=args.lr, alpha_lr=args.alpha_lr,
        ...
        qp_cf_coef=args.qp_cf_coef, qp_lb_coef=args.qp_lb_coef)
...
def main():
    ...
    p.add_argument('--enable_ttc_lateral_gate',action=argparse.BooleanOptionalAction,default=True); p.add_argument('--ttc_lateral_gate',type=float,default=1.8); p.add_argument('--ttc_front_min_longitudinal',type=float,default=0.0); p.add_argument('--ttc_front_max_longitudinal',type=float,default=30.0); p.add_argument('--enable_cbf_lateral_gate',action=argparse.BooleanOptionalAction,default=True); p.add_argument('--cbf_lateral_gate',type=float,default=2.0); p.add_argument('--cbf_front_min_longitudinal',type=float,default=-1.0); p.add_argument('--cbf_front_max_longitudinal',type=float,default=30.0); p.add_argument('--cbf_close_distance_margin',type=float,default=0.3); p.add_argument('--cbf_builtin_policy_name',choices=['idm','lane_change','trajectory_idm','expert','fallback_lane','fallback_obstacle_avoid'],default='idm'); p.add_argument('--cbf_obstacle_radius',type=float,default=1.35); p.add_argument('--cbf_safe_distance',type=float,default=1.35); p.add_argument('--cbf_activation_distance',type=float,default=20.0); p.add_argument('--cbf_ttc_activation_threshold',type=float,default=5.0); p.add_argument('--cbf_min_closing_speed',type=float,default=0.05)
    p.add_argument('--fallback_pass_side',choices=['left','right'],default='left'); p.add_argument('--enable_road_edge_guard',action=argparse.BooleanOptionalAction,default=True); p.add_argument('--road_edge_max_abs_lateral',type=float,default=5.0); p.add_argument('--road_edge_margin',type=float,default=0.35); p.add_argument('--road_edge_soft_margin',type=float,default=0.80); p.add_argument('--road_edge_cost_weight',type=float,default=10.0); p.add_argument('--road_edge_use_start_lane',action=argparse.BooleanOptionalAction,default=True); p.add_argument('--road_edge_lane_width_fallback',type=float,default=3.7); p.add_argument('--road_edge_num_allowed_lane_changes',type=int,default=1); p.add_argument('--road_edge_block_raw',action=argparse.BooleanOptionalAction,default=True); p.add_argument('--fallback_avoid_distance',type=float,default=3.0); p.add_argument('--fallback_max_steer',type=float,default=0.3); p.add_argument('--fallback_min_steer',type=float,default=0.08); p.add_argument('--fallback_target_speed',type=float,default=4.5); p.add_argument('--fallback_speed_kp',type=float,default=0.35); p.add_argument('--fallback_obstacle_lateral_gate',type=float,default=1.5); p.add_argument('--fallback_obstacle_front_min',type=float,default=-2.0); p.add_argument('--fallback_obstacle_front_max',type=float,default=10.0); p.add_argument('--correction_min_throttle',type=float,default=-0.2); p.add_argument('--builtin_action_scale',type=float,default=1.0); p.add_argument('--blend_with_raw',type=float,default=1.0); p.add_argument('--preserve_raw_accel_if_positive',action=argparse.BooleanOptionalAction,default=True)
    p.add_argument('--lr',type=float,default=3e-4); p.add_argument('--qp_cf_coef',type=float,default=0.1); p.add_argument('--qp_lb_coef',type=float,default=0.5); p.add_argument('--alpha_lr',type=float,default=1e-2); p.add_argument('--gamma',type=float,default=0.99); p.add_argument('--gamma_p',type=float,default=0.99); p.add_argument('--sample_k',type=int,default=256); p.add_argument('--lambda_p',type=float,default=0.1); p.add_argument('--lambda_p_warmup_steps',type=int,default=100000); p.add_argument('--use_projection_critic',action='store_true')
    ...
    elif args.filter_type == "cbf_builtin":
        cbf_builtin_cfg = CBFBuiltinFilterConfig(
            dt=args.dt,horizon=args.horizon,steer_limit=args.steer_limit,throttle_limit=args.throttle_limit,brake_limit=args.brake_limit,
            max_dsteer=args.max_dsteer,max_daccel=args.max_daccel,obstacle_radius=args.cbf_obstacle_radius,safe_distance=args.cbf_safe_distance,
            cbf_activation_distance=args.cbf_activation_distance,ttc_activation_threshold=args.cbf_ttc_activation_threshold,min_closing_speed=args.cbf_min_closing_speed,
            builtin_policy_name=args.cbf_builtin_policy_name,
            builtin_action_scale=args.builtin_action_scale,
            ...
            fallback_target_speed=args.fallback_target_speed,
            fallback_speed_kp=args.fallback_speed_kp,
            fallback_obstacle_lateral_gate=args.fallback_obstacle_lateral_gate,
            fallback_obstacle_front_min=args.fallback_obstacle_front_min,
            fallback_obstacle_front_max=args.fallback_obstacle_front_max,
            enable_ttc_lateral_gate=args.enable_ttc_lateral_gate,ttc_lateral_gate=args.ttc_lateral_gate,ttc_front_min_longitudinal=args.ttc_front_min_longitudinal,ttc_front_max_longitudinal=args.ttc_front_max_longitudinal,enable_cbf_lateral_gate=args.enable_cbf_lateral_gate,cbf_lateral_gate=args.cbf_lateral_gate,cbf_front_min_longitudinal=args.cbf_front_min_longitudinal,cbf_front_max_longitudinal=args.cbf_front_max_longitudinal,cbf_close_distance_margin=args.cbf_close_distance_margin,
            enable_road_edge_guard=args.enable_road_edge_guard,road_edge_max_abs_lateral=args.road_edge_max_abs_lateral,road_edge_margin=args.road_edge_margin,road_edge_soft_margin=args.road_edge_soft_margin,road_edge_cost_weight=args.road_edge_cost_weight,road_edge_use_start_lane=args.road_edge_use_start_lane,road_edge_lane_width_fallback=args.road_edge_lane_width_fallback,road_edge_num_allowed_lane_changes=args.road_edge_num_allowed_lane_changes,road_edge_block_raw=args.road_edge_block_raw,
        )
    ...
    env=SafeMetaDriveSampleWrapper(args.env_name,use_filter=args.use_filter,filter_type=args.filter_type,filter_cfg=filter_cfg,mpc_cbf_cfg=mpc_cbf_cfg,cbf_builtin_cfg=cbf_builtin_cfg,terminate_on_safety_violation=args.terminate_on_safety_violation,safety_cost_termination=args.safety_cost_termination,**build_env_kwargs(args)); obs,_=reset_metadrive_env(env,args.seed)
    start_index, num_scenarios = get_metadrive_seed_range(env)
