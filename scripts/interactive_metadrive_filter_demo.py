...
def main():
    ...
    obs,_=env.reset(seed=args.seed); raw=np.zeros(2,dtype=np.float32); use_filter=args.use_filter; step=0
    while True:
        ...
        cv2.imshow('MetaDrive Filter Demo',frame); key=cv2.waitKey(30)&0xFF
        raw*=0.90
