...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", type=str, default="Walker2d-v5") # Ant walker Swimmer halfcheetah reacher pusher
    ...
    parser.add_argument("--num_ent_timesteps", type=int, default=5)
    ...
    parser.add_argument("--total_step", type=int, default=int(1000000))
    ...
    if args.alg == 'sdac':
        ...
        def mish(x: jax.Array):
            return x * jnp.tanh(jax.nn.softplus(x))
        agent, params = create_rf_sac_estient_net(init_network_key, obs_dim, act_dim, hidden_sizes, diffusion_hidden_sizes, mish,
                                          num_timesteps=args.diffusion_steps,
                                          num_timesteps_test=args.diffusion_steps_test,
                                          num_particles=args.num_particles,
                                          noise_scale=args.noise_scale,
        ...
        def mish(x: jax.Array):
            return x * jnp.tanh(jax.nn.softplus(x))
        agent, params = create_mf_sac_ent_net(init_network_key, obs_dim, act_dim, hidden_sizes, diffusion_hidden_sizes, mish,
                                          num_timesteps=args.diffusion_steps,
                                          num_timesteps_test=args.diffusion_steps_test,
                                          num_particles=args.num_particles,
                                          noise_scale=args.noise_scale,
