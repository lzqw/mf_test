...
def main():
 ...
 while True:
  ...
  if s%a.print_every==0:
   diagnostics={'raw_action':getattr(policy,'_safe_filter_last_raw_action',None),'exec_action':getattr(policy,'_safe_filter_last_exec_action',None),**{k:fi.get(k) for k in ['cbf_safe','cbf_active','cbf_violation','predicted_collision','min_pred_dist','min_pred_ttc','min_pred_h_cbf','min_pred_cbf','obstacle_distance','closing_speed','selected_candidate_type','judge_only_cbf','filter_active','projection_residual','builtin_policy_name','builtin_policy_success','builtin_policy_status','builtin_policy_class','builtin_policy_exception']}}
   print(diagnostics)
