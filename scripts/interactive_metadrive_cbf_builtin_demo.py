...
def attach_filter_to_active_policy(env,filt,args):
 ...
 def filtered_act(*a,**kw):
  ...
  if args.use_filter: exec_action,filter_info=filt.project(raw_action,env=env.unwrapped,prev_exec_action=policy._safe_filter_prev_exec_action)
...
def main():
 ...
 while True:
  ...
  if s%a.print_every==0:
   print({'raw_action':getattr(policy,'_safe_filter_last_raw_action',None),'exec_action':getattr(policy,'_safe_filter_last_exec_action',None),**{k:fi.get(k) for k in ['cbf_safe','cbf_active','filter_active','selected_candidate_type','builtin_policy_name','builtin_policy_success','builtin_policy_status','builtin_policy_class','builtin_policy_exception','min_pred_dist','min_pred_ttc','min_pred_h_cbf','min_pred_cbf','cbf_violation','predicted_collision','obstacle_distance','closing_speed','projection_residual']}})
