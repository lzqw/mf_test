...
def parse_args():
 ...
 p.add_argument('--enable_ttc_lateral_gate',action=argparse.BooleanOptionalAction,default=True); p.add_argument('--ttc_lateral_gate',type=float,default=1.8); p.add_argument('--ttc_front_min_longitudinal',type=float,default=0.0); p.add_argument('--ttc_front_max_longitudinal',type=float,default=30.0); p.add_argument('--builtin_policy_name',choices=['idm','lane_change','trajectory_idm','expert','fallback_lane','fallback_obstacle_avoid'],default='idm'); p.add_argument('--require_approaching',action=argparse.BooleanOptionalAction,default=True); p.add_argument('--preserve_raw_accel_if_positive',action=argparse.BooleanOptionalAction,default=False)
 return p.parse_args()
...
def main():
 ...
 while True:
  ...
  if s%a.print_every==0:
   diagnostics={'raw_action':getattr(policy,'_safe_filter_last_raw_action',None),'exec_action':getattr(policy,'_safe_filter_last_exec_action',None),**{k:fi.get(k) for k in ['cbf_safe','cbf_active','cbf_violation','predicted_collision','min_pred_dist','min_pred_ttc','min_pred_h_cbf','min_pred_cbf','obstacle_distance','closing_speed','ttc_relevant','in_ttc_corridor','obstacle_longitudinal','obstacle_lateral','ttc_lateral_gate','selected_candidate_type','judge_only_cbf','filter_active','projection_residual','builtin_policy_name','builtin_policy_success','builtin_policy_status','builtin_policy_class','builtin_policy_exception']}}
   print(diagnostics)
